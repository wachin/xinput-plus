...
from PyQt6.QtCore import (
    Qt, QTimer, QLocale, QTranslator, QLibraryInfo, QCoreApplication, QStandardPaths
)
...
def get_app_icon() -> QIcon:
    """
    Return the application icon, preferring the icon theme (installed) and
    falling back to known file paths (installed and dev).
    """
    # 1) Prefer theme icon (works when .desktop has Icon=xinput-plus and
    #    /usr/share/icons/hicolor/.../xinput-plus.svg is installed).
    themed = QIcon.fromTheme("xinput-plus")
    if not themed.isNull():
        return themed

    # 2) Common installed paths
    candidates = [
...
_TRANSLATOR_REFS: List[QTranslator] = []  # keep refs alive

def _qm_candidates(locale: QLocale) -> List[str]:
    """Candidate .qm names for given locale, e.g. es_ES → xinput-plus_es_ES.qm, xinput-plus_es.qm."""
...
def _i18n_dirs() -> List[Path]:
    """
    Likely locations where .qm files can live:
    - /usr/share/xinput-plus/i18n (installed)
    - /usr/local/share/xinput-plus/i18n (installed)
    - <sys.prefix>/share/xinput-plus/i18n (virtualenv)
    - ./i18n (dev)
    - $PWD/i18n (dev)
    """
    dirs: List[Path] = []
    for base in QStandardPaths.standardLocations(QStandardPaths.StandardLocation.GenericDataLocation):
        dirs.append(Path(base) / APP_NAME / "i18n")
    dirs.append(Path(sys.prefix) / "share" / APP_NAME / "i18n")
    here = Path(__file__).resolve().parent
    dirs.append(here / "i18n")
...
def install_translators(app: QApplication, forced_locale: Optional[str] = None, verbose: bool = False) -> None:
    """
    Load Qt base translations and our app translations from common locations.
    Keep QTranslator objects alive to prevent GC removing translations.
    """
    loc = QLocale(forced_locale) if forced_locale else QLocale.system()

    # Qt base translations (qtbase_*.qm)
    qt_dir = QLibraryInfo.path(QLibraryInfo.LibraryPath.TranslationsPath)
    qt_tr = QTranslator()
    if qt_tr.load(loc, "qtbase", "_", qt_dir):
        app.installTranslator(qt_tr)
        _TRANSLATOR_REFS.append(qt_tr)

    # Our app translations
    loaded_qm = None
    for d in _i18n_dirs():
        for base in _qm_candidates(loc):
            qm = d / base
            if qm.exists():
                tr = QTranslator()
                if tr.load(str(qm)):
                    app.installTranslator(tr)
                    _TRANSLATOR_REFS.append(tr)
                    loaded_qm = str(qm)
                    break
        if loaded_qm:
            break
    ...
    if verbose:
        print(f"[i18n] locale: {loc.name()} (forced={forced_locale or ''})")
        print(f"[i18n] Qt base @ {qt_dir} -> {'ok' if _TRANSLATOR_REFS else 'none'}")
        print(f"[i18n] app qm: {loaded_qm or 'not found'}")
...
class LibinputGUI(QWidget):
    ...
    def __init__(self) -> None:
        ...
        # Window icon: theme → installed paths → dev fallback
    ...
    def build_ui(self) -> None:
        ...
        self.extended_speed_cb = QCheckBox(self.tr("Extra speed (for slow devices)"))
        ...
        row1.addWidget(self.btn_reapply)
